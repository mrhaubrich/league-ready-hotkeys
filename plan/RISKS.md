...
| Event loss or reconnect leaves stale ready-check state | Medium | High | Complement events with reconciliation polling, idempotent transitions, and disconnect cleanup | LCU integration | Mitigated by LRH-017 automated and user Windows validation |
| Cached client survives a League restart or lockfile change | Medium | High | Key cache by lockfile identity/metadata, invalidate on metadata/path or transport failure, then verify restart recovery | LCU integration | Mitigated by LRH-016 automated and user validation |
