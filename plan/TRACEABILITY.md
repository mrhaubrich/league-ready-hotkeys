...
| Bounded low-level input hooks | 8 | LRH-019, LRH-022 | LRH-019 implemented with packed atomic bindings, first-action claim, unrelated-key fast rejection, allocation/lock/log-free callbacks, device-specific hook installation, replacement rollback, and opt-in atomic timing histogram; 40 tests, warnings-denied Clippy, isolated release build, and credential-free registration/replacement/cleanup diagnostic passed; sustained-input and real-ready-check validation remains. LRH-022 remains locked for the diagnostic deadline bound |
