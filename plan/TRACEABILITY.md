...
| Bounded low-level input hooks | 8 | LRH-019, LRH-022 | LRH-019 complete: packed atomic bindings, first-action claim, unrelated-key fast rejection, allocation/lock/log-free callbacks, device-specific hook installation, replacement rollback, and opt-in atomic timing histogram; 40 tests, warnings-denied Clippy, isolated release build, and credential-free registration/replacement/cleanup diagnostic passed; user confirmed the sustained-input and real-ready-check regression scenario on 2026-08-23. LRH-022 is unlocked for the diagnostic deadline bound |
